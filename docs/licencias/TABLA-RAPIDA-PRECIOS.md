# Tabla rápida de precios — bolsillo

Imprime o ten al lado del móvil. Detalle: `00-PRECIOS-REFERENCIA.md` · plan: `PLAN-BIBLIOTECA-Y-PRECIOS.md`.

**Fuente código:** `src/lib/license-quote.ts` → `LICENSE_PRICES` (sync con `functions/lib/`).  
**Canon 2026:** entrada baja + premium real + calculadora por uso (no precio plano único).

---

## Anclas (EUR sin IVA, 2026)

| | Precio |
|--|--------|
| **Micro / un solo uso** (sting, 1 post, 1 vuelo, 1 ep podcast) | **79 €** |
| Comercial · 1 año | **129 €** |
| Comercial · vida del proyecto | **159 €** |
| Comercial · 2 años (lista estándar) | **169 €** |
| + stems | **+59 €** |
| + edit corto | **+49 €** |
| + ads campaña (sobre el plazo) | **+130 €** (ej. 2y → **299 €**) |
| +1 año extra comercial / exclusiva | **+55 €** / **+220 €** |
| Ampliar territorio/medios | **+149 €** |
| Más composición / custom ½ día | **+199 €** |
| Exclusiva media/territorio (desde, 2 años) | **1.200 €** |
| Exclusiva un solo uso / 1 año (suelos) | **890 €** / **1.100 €** |
| Exclusiva fuerte multi-medio | **desde 3.000 €** (rango 3–6k+) |
| + retirar / no disponible catálogo | **+250 €** |
| Buyout catálogo (suelo) | **2.990 €** |
| Buyout alto / a medida | **desde 5.500 €** |
| Personal | petición (**0–49 €** o → comercial) |
| Tema extra mismo proyecto | **~80 %** (−20 %; rango −15–25 %) |
| Indie pro / feature / serie multi (suelo revisión) | **desde 390 €** |
| SaaS / live-ops anual (suelo revisión) | **desde 590 €/año** |
| Broadcast / SVOD (suelo revisión) | **desde 890 €** |

> La **duración del audio** (20 s / 1 min / 3 min) **no** cambia el precio: se cobra el derecho de uso, no el minutaje. Para pistas cortas (stings, hooks) → **micro 79 €**.

**8 variables:** obra · proyecto · medio · territorio · plazo · exclusiva · perfil cliente · entregables.

---

## Por tipo de caso (suelo)

| Te dicen… | Cobras… |
|-----------|---------|
| Sting / hook / 1 post / 1 vuelo | **79** |
| Corto / festival / student (2 años) | **169** |
| Brand film 1 pieza sin ads | **169** |
| Redes marca 1 campaña orgánica | **169** (o 79 si un solo post declarado) |
| Podcast / webserie 1 ep | **79–169** según plazo |
| Indie game 1 título 1 track | **169** |
| App 1 splash/uso fijo | **79–169** |
| Museo / evento 1 vez | **169** |
| Ads Meta/YouTube campaña | **base + 130** (2y → **299**) |
| Serie varios eps / feature indie | pack **390–690** o revisión |
| TV regional / SVOD 1 título | **desde 890–1.800** |
| Live-ops / SaaS ilimitado | **590–1.200+/año** |
| Tour multi-ciudad | **390–1.500** |
| “Nadie más la use” (media/territorio) | **≥ 1.200** |
| Exclusiva fuerte multi-medio | **3.000–6.000+** |
| Forever + saca del catálogo | **≥ 2.990** |
| Reventa / stock / template reventa | **No** |
| Post hobby con marca/dinero | **≥ 79** comercial (no personal) |

---

## Script de respuesta (30 s)

1. ¿Dinero / marca / cliente? → mín. comercial (no personal).  
2. ¿Un post / un vuelo / un sting? → **79** (micro).  
3. ¿Ads de pago? → base del plazo + **130**.  
4. ¿Exclusiva / buyout? → **≥ 1.200** / **≥ 2.990**.  
5. ¿2 años 1 obra 1 proyecto? → **169** (+59 stems).  
6. ¿Multi / live / SaaS / TV gorda? → revisión (suelos arriba).

---

## Posicionamiento (una línea)

**MIDI manual sin IA + stems + clearance directo** — premium indie, no stock a 29 €.

---

## Correo

- **From automático:** `licencias@nimpo3dstudio.com`  
- **Estudio / Reply-To cliente:** `contacto@nimpo3dstudio.com` (o `QUOTE_TO_EMAIL`)
